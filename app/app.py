...
# Phase 3: Streamlit Web Interface (replaces Flask + HTML/CSS/JS)
...
# ── Custom CSS (minimal, keeps Streamlit look) ────────────────────────────────
st.markdown("""
    <style>
...
if model is None:
    st.warning(
        "⚠️ Trained model not found at `results/models/baseline_best.pth`. "
...
with st.sidebar:
    st.header("ℹ️ About")
    st.markdown("""
**FedMed** uses a ResNet-50 model trained on the Shenzhen Hospital chest X-ray dataset 
(336 images) using Federated Learning with the Flower framework.
...
**Disclaimer:** This tool is for research purposes only and is not a substitute for clinical diagnosis.
    """)
    st.divider()
    ...
    if st.button("🔬 Analyze X-Ray", type="primary", use_container_width=True):
        with st.spinner("Analyzing image…"):
            try:
                ...
                # Confidence bar chart for both classes
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown('<p class="footer">FedMed — Privacy-Preserving TB Detection | Research Project 2025</p>', unsafe_allow_html=True)